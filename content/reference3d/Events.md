...
class ClickedEvent extends Event {
  ...
  intersection?: THREE.Intersection; // точка пересечения с отрисованным объектом на сцене при клике по объекту
}
...
class HoverEvent extends Event {
  modelId: string; // идентификатор модели
  modelElementId: string; // идентификатор элемента модели
  intersection?: THREE.Intersection; // точка пересечения с отрисованным объектом на сцене при ховере объекта
}
